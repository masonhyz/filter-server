...
Bitmap *read_header() {
    ...
    if (fread(temp, sizeof(unsigned char), BMP_HEADER_SIZE_OFFSET, stdin) != BMP_HEADER_SIZE_OFFSET) {
        fprintf(stderr, "End of file reached.\n");
        exit(1);
    }

    Bitmap *bmp_ptr = malloc(sizeof(Bitmap));
    if (bmp_ptr == NULL) {
        perror("malloc");
        exit(1);
    }

    if (fread(&(bmp_ptr->headerSize), sizeof(int), 1, stdin) != 1) {
        fprintf(stderr, "End of file reached.\n");
        free(bmp_ptr);
        exit(1);
    }

    bmp_ptr->header = malloc(bmp_ptr->headerSize);
    if (bmp_ptr->header == NULL) {
        perror("malloc");
        free(bmp_ptr);
        exit(1);
    }

    memcpy(bmp_ptr->header, temp, BMP_HEADER_SIZE_OFFSET);
    memcpy(bmp_ptr->header + BMP_HEADER_SIZE_OFFSET, &(bmp_ptr->headerSize), sizeof(int));

    if (fread(bmp_ptr->header + BMP_HEADER_SIZE_OFFSET + sizeof(int), sizeof(unsigned char), BMP_WIDTH_OFFSET - BMP_HEADER_SIZE_OFFSET - sizeof(int), stdin) != (BMP_WIDTH_OFFSET - BMP_HEADER_SIZE_OFFSET - sizeof(int))) {
        fprintf(stderr, "End of file reached.\n");
        free(bmp_ptr->header);
        free(bmp_ptr);
        exit(1);
    ...
    if (fread(&(bmp_ptr->width), sizeof(int), 1, stdin) != 1) {
        fprintf(stderr, "End of file reached.\n");
        free(bmp_ptr->header);
        free(bmp_ptr);
        exit(1);
    ...
    if (fread(&(bmp_ptr->height), sizeof(int), 1, stdin) != 1) {
        fprintf(stderr, "End of file reached.\n");
        free(bmp_ptr->header);
        free(bmp_ptr);
        exit(1);
    ...
    memcpy(bmp_ptr->header + BMP_WIDTH_OFFSET, &(bmp_ptr->width), sizeof(int));
    memcpy(bmp_ptr->header + BMP_HEIGHT_OFFSET, &(bmp_ptr->height), sizeof(int));

    if (fread(bmp_ptr->header + BMP_HEIGHT_OFFSET + sizeof(int), sizeof(unsigned char), bmp_ptr->headerSize - BMP_HEIGHT_OFFSET - sizeof(int), stdin) != (bmp_ptr->headerSize - BMP_HEIGHT_OFFSET - sizeof(int))) {
        fprintf(stderr, "End of file reached.\n");
        free(bmp_ptr->header);
        free(bmp_ptr);
        exit(1);
...
void scale(Bitmap *bmp, int scale_factor) {
    int new_size = scale_factor * scale_factor * bmp->height * bmp->width * 3 + bmp->headerSize;
    int new_width = bmp->width * scale_factor;
    int new_height = bmp->height * scale_factor;
    memcpy(bmp->header + BMP_FILE_SIZE_OFFSET, &new_size, sizeof(int));
