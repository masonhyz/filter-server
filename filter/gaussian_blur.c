...
void gaussian_blur(Bitmap *bmp) {
    ...
    Pixel p = apply_gaussian_kernel(row0, row1, row2);
    if (fwrite(&p, sizeof(Pixel), 1, stdout) != 1) {
        perror("fwrite");
        exit(1);
    }
    for (int j = 0; j < width - 2; j++) {
        p = apply_gaussian_kernel(row0 + j, row1 + j, row2 + j);
        if (fwrite(&p, sizeof(Pixel), 1, stdout) != 1) {
            perror("fwrite");
            exit(1);
        }
    }
    p = apply_gaussian_kernel(row0 + width - 3, row1 + width - 3, row2 + width - 3);
    if (fwrite(&p, sizeof(Pixel), 1, stdout) != 1) {
        perror("fwrite");
        exit(1);
    }
